...
CostLoggingDebug := false ; if true, makes lots of cost images. false saves disk space.
TimingDebug := false ; if true, writes how long each part of the software takes. False leaves cleaner log files
PostMoveWait := 150 ; miliseconds. depends on your client speed. Increase if the ritual finder doesn't always mouseover the top left of each item
PostCopyWait := 50 ; miliseconds. depends on your client speed. Increase if the ritual finder stops partway through
...
F3::{
  if WinActive("Path of Exile"){
    ...
    FileAppend 'Created with RitualReaderv0.5.4`r`nhttps://github.com/nerdyjoe314/RitualReader/`r`n', LogName
...
; move mouse to the right location, wait for the game to catch up, and copy item info
      ...
      A_Clipboard := "-1"
      Sleep PostMoveWait ; depends on latency 
      Send "^c"
      ClipWait(0.05)
      newClip := A_Clipboard
      preFound := 0
; if the copy hasn't finished yet, take this time to check for the tribute symbol above the cursor. We can use this info later
      if (newClip=-1){
        bounds := [xTLCorner+thisX*xStep-xStep, yTLCorner+thisY*yStep-120, xTLCorner+thisX*xStep+xStep, yTLCorner+thisY*yStep]
; find the tribute symbol
        ...
        Sleep PostCopyWait ; depends on latency 
; grab the clipboard again, after spending time looking for the symbol
        newClip := A_Clipboard
      }
...
; if you didn't find the tribute symbol yet, do a more thorough search
      if (preFound = 0){
...
; if you didn't find the tribute symbol, save the image and clear cost
      ...
      if TimingDebug {
        FileAppend "Total time: " costTime-startTime "`r`nDefer time: " deferTime-startTime "`r`n", LogName
